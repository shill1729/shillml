...
from shillml.losses import AELoss, DiffusionLoss, DriftMSELoss
...
if __name__ == "__main__":
    ...
    x, _, mu, cov, _ = cloud.generate(num_points) # returns points, weights, drifts, cov, local coord
    x, mu, cov, p, orthogcomp = process_data(x, mu, cov, d=2)

    # Define AE model
    ae = AE(input_dim, latent_dim, hidden_layers, activation(), activation())
    ae_loss = AELoss()
    fit_model(ae, ae_loss, x, lr=lr, epochs=epochs_ae, batch_size=batch_size)
    ...
    fit_model(model_drift, drift_loss, x, mu, epochs=epochs_sde, batch_size=batch_size)

    # Compute test loss
    x, _, mu, cov, _, = cloud.generate(num_test, seed=None)
    x, mu, cov, p, orthogcomp = process_data(x, mu, cov, d=2)
    dpi = ae.encoder.jacobian_network(x).detach()
    encoded_cov = torch.bmm(torch.bmm(dpi, cov), dpi.mT)
    dl = diffusion_loss.forward(model_diffusion, x, (mu, cov, encoded_cov))
    drl = drift_loss(model_drift, x, mu)
    aeloss = ae_loss.forward(ae, x)
