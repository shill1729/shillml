...
class CTBAE(AutoEncoder):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        """
            Forward pass of the Contractive Tangent Bundle Autoencoder.

            Args:
                x (Tensor): Input tensor with shape (batch_size, extrinsic_dim).

            Returns: Tuple[Tensor, Tensor]: A tuple containing:
            - x_hat (Tensor): Reconstructed input with shape (
            batch_size, extrinsic_dim).
            - dpi (Tensor): jacobian of the encoder, a tensor with shape (batch_size, intrinsic_dim, extrinsic_dim)
            - model_projection (Tensor): Projection of the latent representation on the
...
if __name__ == "__main__":
    ...
    # Define model
    ae = CTBAE(3, 2, [64], nn.Tanh(), nn.Tanh())
    ae_loss = CTBAELoss(tangent_bundle_weight=0.01, contractive_weight=0.001, norm="fro")
    # Fit the model
